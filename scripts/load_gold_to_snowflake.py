...
def run_gold_to_snowflake(**context):
    gold_file = context["ti"].xcom_pull(
        key="gold_file", task_ids="gold_file"
    )
    if not gold_file:
        raise ValueError("Gold file path not found in XCom for date: {}".format(context['ds_nodash']))
    
    execution_date = context['data_interval_start'].strftime("%Y-%m-%d %H:%M:%S")

    df = pd.read_csv(gold_file)

    # Récupérer les informations de connexion à Snowflake depuis Airflow Connections
